---
title: Confidentialité et cookies
description: Comment Au centime près traite vos données personnelles et utilise les cookies.
---
Dernière mise à jour : septembre 2026.

## Responsable du traitement

Sonia, éditrice du site (voir les [mentions légales](/mentions-legales/)).

## Données collectées

- **Messages envoyés par e-mail** : votre adresse et votre message servent uniquement à vous répondre et sont conservés 12 mois au maximum. Ils ne sont jamais vendus.
- **Mesure d'audience** : des statistiques de visite anonymisées, pour savoir quels articles sont lus.
- **Publicité** : le site affiche des annonces diffusées par **Google AdSense**.

## Publicité et cookies

Google et ses partenaires peuvent utiliser des cookies pour afficher des annonces, qui sont personnalisées **seulement si vous l'acceptez**. À votre première visite, un bandeau de consentement vous permet d'accepter ou de refuser ces cookies. Vous pouvez modifier votre choix à tout moment.

Pour en savoir plus sur l'usage de vos données par Google : [Comment Google utilise les données des sites partenaires](https://policies.google.com/technologies/partner-sites). Vous pouvez aussi gérer la personnalisation des annonces sur [adssettings.google.com](https://adssettings.google.com).

## Vos droits

Conformément au RGPD, vous pouvez accéder à vos données, les faire rectifier ou effacer, et vous opposer à leur traitement. Pour cela, écrivez à [alsnovagestion@gmail.com](mailto:alsnovagestion@gmail.com). Vous pouvez aussi adresser une réclamation à la **CNIL** ([cnil.fr](https://www.cnil.fr)).

## Hébergement des données

Le site est hébergé par GitHub Pages (GitHub, Inc., États-Unis), dans le cadre des garanties prévues par le RGPD pour les transferts hors de l'Union européenne.
